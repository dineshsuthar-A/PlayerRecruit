...
export default function ChatUserListItem(props) {
    ...
    const d = new Date();
    return (
        <View style={{ paddingVertical: windowHeight * 0.015, marginBottom: 2, borderRadius: windowHeight * 0.01, backgroundColor: props?.count && props.count > 0 ? '#F9FAFC' : 'rgb(250, 250, 250)', borderRadius: 5, borderWidth: props?.count && props?.count > 0 ? 1 : null, borderColor: props?.count && props?.count > 0 ? "#EFF2F7" : null, marginHorizontal: 10 }}>
